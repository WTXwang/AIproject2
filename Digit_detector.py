...
def predict_image(img_path, model, show=False):
    ...
    for digit_img in digits:
        ...
        results.append((pred, max_prob))
    # 选择概率最大的结果
    best_pred, best_prob = max(results, key=lambda x: x[1])
    if show:
        print(f'所有分割数字预测结果: {results}')
...
def gui_predict():
    ...
    def select_image():
        ...
        if file_path:
            ...
            result_var.set('正在识别，请稍候...')
            root.update_idletasks()
            pred, prob = predict_image(file_path, model=model, show=False)
            if pred is not None:
                result_var.set(f'识别结果: {pred}  概率: {prob:.4f}')
            else:
                result_var.set('未检测到数字')
    root = tk.Tk()
    root.title('手写数字识别(Keras)')
    root.geometry('600x500')
    root.configure(bg='#f0f0f0')
    common_font = ('Arial', 16)
    title_label = tk.Label(root, text='手写数字识别', font=('Arial', 28, 'bold'), bg='#f0f0f0')
    title_label.pack(pady=20)
    btn = tk.Button(root, text='选择图片', command=select_image, font=common_font, width=15, bg='#4a90e2', fg='white')
    btn.pack(pady=10)
    img_label = tk.Label(root, bg='#d9d9d9', width=280, height=280)
    img_label.pack(pady=20)
    result_var = tk.StringVar()
    # 结果字体加大加粗，颜色突出
    result_label = tk.Label(root, textvariable=result_var, font=('Arial', 26, 'bold'), bg='#f0f0f0', fg='#d0021b')
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        print('正在识别，请稍候...')
        # 调用预测函数
        pred, prob = predict_image(img_path, model=model)
        if pred is not None:
            print(f'图片中概率最大的数字预测为: {pred}，概率: {prob:.4f}')
        else:
            print('未检测到数字')
    else:
        gui_predict()
